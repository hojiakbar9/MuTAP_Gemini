...
def call_prompt_on_model(
    ...
    explanation_completion = response.text

    print(reruns_if_empty)
    print(reruns_no_assert)
    if int(reruns_if_empty) > 0 and explanation_completion.strip() == "":
        print("empty output")
    ...
    if int(reruns_no_assert) > 0 and not re.search("assert", explanation_completion):
...
def call_LLMs(initial_prompt, stop_sequences, max_output_tokens):
    output = call_prompt_on_model(initial_prompt, stop_sequences, max_output_tokens)
...
def generate_test_oracle_intial(prmpt, DATASET, SCRIPT, name_string, output_string):
    ...
    os.makedirs(
        os.path.join(CODE_DIR, SCRIPT, "Gemini"),
        exist_ok=True,
    )

    OUTPUT_NAME = output_string + SCRIPT + ".py"
    OUTPUT_PATH = os.path.join(CODE_DIR, SCRIPT, "Gemini", OUTPUT_NAME)
...
def apply_syntax_fix(DATASET, SCRIPT, name_string, output_string, csv_name):
    ...
    script_name = name_string + SCRIPT + ".py"
    input_path = os.path.join(CODE_DIR, SCRIPT, "Gemini", script_name)
    ...
    correct_function, synx_LLMs, synx_split = check_test_oracle_synx(
        function_to_correct, 3
    )
    ...
    if len(arguments) != 7:
        ...
        if os.path.splitext(syntax_report)[1] == ".csv":
            if DATASET == "HumanEval":
                generate_test_oracle_intial(
                    prompt_type, DATASET, str(task_num), script_name, "test_oracle_NDS_"
                )
                apply_syntax_fix(
                    DATASET,
                    str(task_num),
                ...
                generate_test_oracle_student(
                    prompt_type, DATASET, SCRIPT_num, script_name, "test_oracle_NDS_"
                )
                apply_syntax_fix(
                    DATASET,
                    SCRIPT_num,
