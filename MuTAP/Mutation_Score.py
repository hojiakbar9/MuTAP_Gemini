...
def MS_generator(DATASET, SCRIPT, script_string, csv_name):
    ...
    if DATASET == "Refactory":
        ...
        SCRIPT_PATH = os.path.join(CODE_DIR, "Tests", SCRIPT, "Gemini", SCRIPT_NAME)
    else:
        CODE_DIR = os.path.join(
            os.getcwd(),
            DATASET,
        ...
        SCRIPT_NAME = script_string + SCRIPT + ".py"
        SCRIPT_PATH = os.path.join(CODE_DIR, SCRIPT, "Gemini", SCRIPT_NAME)
    ...
    try:
        ...
        for mut_path in lst_mut:
            ...
            try:
                temp_dir_path_script = os.path.join(
                    temp_dir_path, "temp_test_script.py"
                )

                result = subprocess.run(
                    ["python", temp_dir_path_script],
        ...
        new_row = pd.DataFrame(
            {
                "name": [SCRIPT_NAME],
                "MS": [Mutation_score],
                "lst_survived": [survived_mutants],
                "n_killed": [n_killed_mutants],
                "n_all_mut": [all_mutants],
                "is_problematic": [problematic_test],
                "lst_problematic": [problematic_list],
                "lst_timeout": [timout_list],
                "lst_killed": [killed_list],
            }
        )

        if not new_row.isna().all().all():
            result_dt = pd.concat([result_dt, new_row], ignore_index=True)
...
def main():
    ...
    if len(arguments) != 5:
        raise SystemExit(
            "5 inputs are required: Mutation_Score.py [dataset] [task_num] [testfile prefix name] [csv report filename (.csv)]"
        )
    else:
        DATASET = arguments[1]
