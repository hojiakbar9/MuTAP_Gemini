...
def directory_crawler(directory_to_crawl) -> dict:
    ...
    for question in list_questions:
        ...
        for root, dirs, files in os.walk(
            os.path.join(os.getcwd(), directory_to_crawl_question)
        ):
...
def filter_greedily(script_dirs: list, assert_dict: dict) -> list:
    ...
    file_to_run = [
        x for x in os.listdir("/".join(mother_dir)) if x.startswith("script_NDS_")
    ][0]
    ...
    results_path = os.path.join(
        ...
        "Reference_Scripts",
        "Tests",
        "greedy_NDS_mut_all_results.csv",
    )
...
def main():
    ...
    if len(arguments) != 7:
        raise SystemExit(
            "7 inputs are required: greedy_test_generator.py [dataset] [csv report filename (.csv)]"
        )
    else:
        DATASET = arguments[1]
        greedy_report = arguments[2]
        if DATASET == "HumanEval":
            esults_path = os.path.join(
                os.getcwd(), DATASET, "Testing_HumanEval", greedy_report
            )

        elif DATASET == "Refactory":
            results_path = os.path.join(
                os.getcwd(), DATASET, "Reference_Scripts", "Tests", greedy_report
            )
