        ...
        if ln.startswith("assert"):
            ...
            try:
                result = subprocess.run(
                    ["python3", f"{os.path.join(temp_dir_path,'temp_test_script.py')}"],
...
def main():
    ...
    if len(arguments) != 6:
        raise SystemExit(
            "6 inputs are required: semantic_err_correction.py [dataset] [task_num] [input prefix name] [output prefix name] [csv report filename (.csv)]"
        )
    else:
        DATASET = arguments[1]
        ...
        if os.path.splitext(semantic_report)[1] == ".csv":
            if DATASET == "HumanEval":
                ...
                input_path = os.path.join(CODE_DIR, task_num, "Gemini", script_name)
                if os.path.exists(input_path):
                    apply_semantix_fix(
                        DATASET, task_num, input_string, output_string, semantic_report
                    )
                ...
                input_path = os.path.join(CODE_DIR, SCRIPT_num, "Gemini", script_name)
                if os.path.exists(input_path):
                    apply_semantix_fix(
                        DATASET,
                        SCRIPT_num,
