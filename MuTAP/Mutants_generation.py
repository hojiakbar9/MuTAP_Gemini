...
def Generate_Mutants(DATASET, num, input_string):
    if DATASET == "HumanEval":
        ...
        test_path = os.path.join(CODE_DIR, "Gemini", TEST_NAME)
...
def main():
    ...
    if len(arguments) != 5:
        raise SystemExit(
            "5 inputs are required: Mutants_generation.py [dataset] [task_num] [input prefix name] [csv report filename (.csv)]"
        )
    else:
        DATASET = arguments[1]
