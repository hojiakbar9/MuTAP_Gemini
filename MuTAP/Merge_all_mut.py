...
def Mut_all_in_one(DATASET, SCRIPT, code_string, script_string, output_string):
    ...
    Script_path = os.path.join(CODE_DIR, SCRIPT, "Gemini")

    code_name = code_string + SCRIPT + ".py"
    code_path = os.path.join(CODE_DIR, SCRIPT, "Gemini", code_name)
    ...
    for script in lst_script:
        ...
        if script_name.startswith(script_string):
            ...
            if single_test_content != "":
                test_content_after_mut = (
                    test_content_after_mut + merge_asserts(script) + "\n"
                )
...
def main():
    ...
    if len(arguments) != 6:
        raise SystemExit(
            "6 inputs are required: Merge_all_mut.py [dataset] [task_num] [inital test name] [augmented test name] [output name]"
        )
    else:
        DATASET = arguments[1]
