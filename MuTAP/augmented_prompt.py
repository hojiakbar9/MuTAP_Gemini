...
def prompt_augmentation(
    prmpt, DATASET, num, input_string, output_string, mut_report_name
):
    SCRIPT = str(num)
    if DATASET == "HumanEval":
        ...
        input_path = os.path.join(CODE_DIR_HE, SCRIPT, "Gemini", script_name)
        ...
        input_path = os.path.join(CODE_DIR_RF, SCRIPT_num, "Gemini", script_name)

        mut_dt = read_csv(os.path.join(CODE_DIR_RF, mut_report_name))
    mut_dt["lst_survived"] = mut_dt["lst_survived"].apply(
    ...
    if PUT_inx >= 0:
        ...
        if row["MS"][0] < 1 and row["is_problematic"][0] != 2:

            with open(input_path) as input:
                function_to_test = input.read()

            lst_mut = row["lst_survived"][0]
            j = 0
            for mut in lst_mut:
                if DATASET == "HumanEval":
                ...
                if prmpt == "normal":
                    mutant_prompt = mutatePromptGenerator(
                        function_to_test, mutant_content
                    )
                    ...
                    mutant_prompt = fewshotMutantPromptGenerator(
                        fewshot_content, function_to_test, mutant_content
                    )
                ...
                if DATASET == "HumanEval":
                    ...
                    output_path = os.path.join(
                        CODE_DIR_HE, SCRIPT, "Gemini", OUTPUT_NAME
                    )
                elif DATASET == "Refactory":
                    OUTPUT_NAME = output_string + SCRIPT_num + "_" + str(j) + ".py"
                    output_path = os.path.join(
                        CODE_DIR_RF, SCRIPT_num, "Gemini", OUTPUT_NAME
                    )
...
def main():
    ...
    if len(arguments) != 7:
        raise SystemExit(
            "7 inputs are required: augmented_prompt.py [prompt type] [dataset] [task_num] [input prefix name] [output prefix name] [csv report filename (.csv)]"
        )
    else:
